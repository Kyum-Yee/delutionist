...
| `prepare_mini_step1_workers` | **[Mini]** Step 1 워커 준비. `executor="GEMINI"`(기본, cmd 반환) / `executor="SELF"`(orchestrator 직접 사용 — prompt 본문 반환). 자세한 흐름은 §18 |
| `prepare_mini_step1_1_workers` | **[Mini]** Step 1-1 워커 준비 (PPB 1/5 폐기 + 아이디어 변환). `GEMINI`/`SELF` 모두 지원 |
| `parse_mini_step1_response` | **[Mini]** Gemini 응답에서 chain 줄 추출 (3자리 숫자로 시작하는 줄만) |
| `parse_mini_step1_1_response` | **[Mini]** Gemini 응답에서 idea 줄 추출 (번호·따옴표·마크다운 볼드 제거) |
...
```
delusionist_factory_personal/mini/
...
├── staging/            # MCP 호출 시 워커 prompt 파일 저장 위치
